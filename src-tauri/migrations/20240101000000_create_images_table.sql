-- ╔══════════════════════════════════════════════════════════════════════════╗
-- ║  FROZEN AT v1.0. DO NOT EDIT THIS FILE.                                   ║
-- ║  Every schema change from here is a NEW numbered migration file.          ║
-- ╚══════════════════════════════════════════════════════════════════════════╝
--
-- Why this is absolute, and not a style preference:
--
-- `sqlx::migrate!()` stores a CHECKSUM of every migration in the library's own
-- `_sqlx_migrations` table. Editing this file changes its checksum, so the next
-- launch compares the new checksum against the one recorded in a `library.db`
-- that already exists, finds they differ, and refuses to migrate. The library
-- does not open. Not "opens with a warning" — does not open, permanently, with
-- no in-app path back.
--
-- Until v1.0 shipped, that cost nothing: every library was a development one and
-- "recreate it" was a valid answer. It stopped being valid the moment somebody
-- else's photo library depended on this file staying byte-identical.
--
-- So: to add a column, a table or an index, create
-- `migrations/<timestamp>_<name>.sql` beside this one. sqlx applies it in order
-- and records it separately, and libraries at any prior version roll forward.
--
-- `db.rs` surfaces a checksum failure as `AppError::LibraryVersion` with a
-- message a user can act on, and takes a `library.db.bak` snapshot before every
-- migration run. Both are recovery aids, not permission to edit this file.
...
-- The exact counterpart of idx_assets_tags_reverse, for the exact same reason.
-- The composite PK and both indexes above lead with folder_id, so they serve
-- folder -> assets. Nothing led with asset_id, which made EVERY asset -> folders
-- question a full scan of this table:
--
--   * Scope::Uncategorized and Condition::Uncategorized — `a.id NOT IN (SELECT
--     asset_id FROM assets_folders)`, run on every manifest load of that scope;
--   * folder_membership, on every selection change (the inspector's tri-state);
--   * all_folder_members / clear_folder_members, on every SetFolders step;
--   * the folder_text and folder_note subqueries in search::INDEX_SELECT, which
--     is twice per row on a full reindex.
CREATE INDEX IF NOT EXISTS idx_assets_folders_reverse ON assets_folders (asset_id, folder_id);
