<script lang="ts">
    import { Minus, ArrowRight, Filter } from "@lucide/svelte";
    import { assetLibrary, type DateField, type Folder } from "$lib/assets.svelte";
    ...
    /**
     * The live preview: this pattern, against real assets.
     *
     * The whole reason rename gets a preview when no other step does — a pattern
     * is a tiny program, and reading one is not the same as knowing what it
     * produces. It renders through the SAME Rust path the run uses, so it cannot
     * disagree with the result.
     *
     * Falls back to a few of the library's own assets when nothing is selected,
     * because an action is normally written before it's used.
     */
    ...
    let previewToken = 0;
    $effect(() => {
        if (step.op.type !== "rename_with_pattern") return;
        const snapshot = $state.snapshot(step) as Step;
        const ids = selection.assetIds;
        const token = ++previewToken;
        // Debounced, and superseded by token: a slow render from three
        // keystrokes ago must not overwrite the answer for what's on screen.
        const timer = setTimeout(async () => {
            try {
                const preview = await assetLibrary.previewRename(snapshot, ids, 3);
...
<div class="flex flex-col gap-2 rounded border border-neutral-800 p-2">
  <div class="flex items-start gap-1.5">
    ...
    {:else if step.op.type === "rename_with_pattern"}
        <!-- Captured here because the {#each} callback below is a closure, and
             the narrowing from this branch doesn't reach inside it. -->
        {@const pattern = step.op.pattern}
        <div class="flex min-w-0 flex-1 flex-col gap-1.5">
            <input
                type="text"
                value={step.op.pattern}
                oninput={(e) => setOp({ ...step.op, pattern: e.currentTarget.value } as Op)}
            ...
            <!-- Click to insert. Typing `{index}` correctly from memory is not a
                 skill worth requiring, and a mistyped token is a hard error. -->
            <div class="flex flex-wrap gap-1">
                {#each RENAME_TOKENS as { token, hint } (token)}
                    <button
                        ...
                        title={hint}
                        onclick={() =>
                            setOp({ ...step.op, pattern: pattern + token } as Op)}
                        class="rounded bg-neutral-800 px-1.5 py-0.5 font-mono text-[10px]
                               text-neutral-400 transition-colors hover:bg-neutral-700
            ...
            <!-- The knobs {index} and {date} depend on. Visible rather than
                 buried, because they're the difference between an action that
                 renumbers consistently and one that doesn't. -->
            <div class="flex flex-wrap items-center gap-1.5 text-[10px] text-neutral-500">
                <span>number by</span>
                <select
                    value={step.op.index_order}
                    onchange={(e) =>
                        setOp({
                            ...step.op,
                            index_order: e.currentTarget.value as RenameOrder,
                        } as Op)}
                ...
                <select
                    value={step.op.index_ascending ? "asc" : "desc"}
                    onchange={(e) =>
                        setOp({
                            ...step.op,
                            index_ascending: e.currentTarget.value === "asc",
                        } as Op)}
                ...
                <input
                    type="number"
                    min="0"
                    value={step.op.index_start}
                    oninput={(e) =>
                        setOp({
                            ...step.op,
                            index_start: e.currentTarget.valueAsNumber || 0,
                        } as Op)}
                ...
                <span>· date is</span>
                <select
                    value={step.op.date_field}
                    onchange={(e) =>
                        setOp({
                            ...step.op,
                            date_field: e.currentTarget.value as DateField,
                        } as Op)}
